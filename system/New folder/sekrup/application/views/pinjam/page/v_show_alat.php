    ...
    <div class="row">
      ...
      <div class="card">
          <div class="card-header d-flex justify-content-end align-items-right">
            <a class="btn btn-info" href="<?php echo base_url('riwayatservisalat'); ?>">
            <i class="fas fa-arrow-left"></i> Kembali
            </a>
            <a class="btn btn-primary" href="<?php echo base_url('riwayatservisalat/tambah/' . $id_alat); ?>">
            ...
            <div class="table-responsive">
              <table class="table table-striped" id="table-1">
                ...
                <tbody class="text-center">
                  <?php if (!empty($rs_alat) && (is_array($rs_alat) || is_object($rs_alat))): ?>
                    <?php $no = 1; foreach ($rs_alat as $key): ?>
                      <tr>
                        <td class="text-center"><?php echo $no++; ?></td>
                        <td><?php echo isset($key->tgl_servis) ? $key->tgl_servis : 'No Data'; ?></td>
                        <td><?php echo isset($key->ket) ? $key->ket : 'No Data'; ?></td>
                        <td>
                          <a href="<?php echo base_url('riwayatservisalat/edit/' . $key->id); ?>" class="btn btn-icon icon-left btn-primary"><i class="fas fa-edit"></i> Edit</a>
                          <a href="<?php echo base_url('riwayatservisalat/delete/' . $key->id); ?>" class="btn btn-icon icon-left btn-danger" onclick="return confirm('Anda yakin ingin menghapus data ini?')"><i class="fas fa-trash"></i> Hapus</a>
                        </td>
                      </tr>
                    <?php endforeach;?>
                  <?php else: ?>
