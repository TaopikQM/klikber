              ...
              <table class="table table-striped" id="table-1">
                ...
                <tbody>
                  <?php
                    ...
                    echo "</pre>";*/
                  ?>
                  <?php $i=1; foreach ($dataruangan as $key) { ?>
                      <tr>
                          ...
                          <td class="text-center">
                            <?php 
                              $akhir='.00';
                              $jmin= $key->timein."$akhir";
                              $jmot= $key->timeot."$akhir";
                              $timein=date_create(date($jmin));
                              $timeot=date_create(date($jmot));
                              $jmjam=date_diff($timein,$timeot);
                            ?>
                              Durasi peminjaman selama <?php echo $jmjam->format("%h jam");?> <br>Mulai Jam<br>
                            <?php echo date($jmin);?><br>s/d<br><?php echo date($jmot);?><br>
                          </td>
                          ...
                          <td class="text-center" >
                            <?php
                              if ($key->sts_pjm==1) { ?>
                            <a href="<?php $data=$this->encryption->encrypt(base64_encode($key->id_pjm)); $ff=str_replace(array('+','/','='),array('-','_','~'),$data); echo base_url();?>pinjam/cetak_pinjam_ruang/<?php echo $ff; ?>" target="_blank">
                            ...
                            <!-- <button type="button" class="btn btn-success btn-sm" data-toggle="tooltip" title="Upload Dokumen Untuk TTE">
                             ...
                             <a href="<?php   
                                $data=$this->encryption->encrypt(base64_encode($key->id_pjm)); $ff=str_replace(array('+','/','='),array('-','_','~'),$data); echo base_url();?>pinjam/edit_pinjaman_ruangan/<?php echo $ff; ?>">
                            ...
                            <button type="button" class="btn btn-primary btn-sm" data-toggle="tooltip" id="ubahst" data-nopin="<?php echo $key->nopinjam;?>"  data-id="<?php echo $key->id_pjm;?>" data-status="<?php echo $key->sts_pjm?>"title="Ubah Status Peminjaman">
                              <i  class="fa fa-upload" aria-hidden="true"></i>
                            </button>
                            <button type="button" class="btn btn-danger btn-sm" data-toggle="tooltip" title="Hapus Nomor" id="hapus" data-nopin="<?php echo $key->nopinjam;?>" data-id="<?php echo $key->id_pjm;?>" data-nmruang="<?php echo $key->nmruang;?>">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer bg-whitesmoke br">
         <?php echo form_open_multipart('pinjam/hapus_pinjam_ruang');?>
            <input type="hidden" name="nopin" value="" id="id_del">
...
<div class="modal fade" id="modal_status" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <?php echo form_open_multipart('pinjam/ubah_status_pinjam_ruang');?>
      <div class="modal-body">
      ...
      <div class="modal-footer bg-whitesmoke br">
            <input type="hidden" name="nopin" value="" id="id_st">
...
$(document).on('click','#hapus',function(){
        ...
        var id=$(this).data('id');
        var hal=$(this).data('nmruang');
        var text="Hapus Nomor Peminjaman : "+no;
        var tgh= "Nama Ruangan : "+hal;
        $("#mp-tex").text(text);
        $("#mp-tex1").text(tgh);
        $("#id_del").val(no);
});
...
$(document).ready(function(){
  $(document).on('click','#ubahst',function(){
          ...
          var text="Ubah Status Peminjaman : "+no;
