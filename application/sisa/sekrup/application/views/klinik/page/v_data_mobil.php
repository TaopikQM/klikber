      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <a href="<?php echo base_url()?>poli/create" >
              ...
              <table class="table table-striped table-hover" id="table-1" style="width:100%;" >
                ...
                <tbody>
                  <?php
                    ...
                    echo "</pre>";*/
                  ?>
                  <?php $nomori=1; foreach ($roles as $key) { ?>
                      <tr bgcolor="#cccccc">
                          <td  class="text-center"><?php echo $nomori;?></td>
                          <td  class="text-center">
                            <?php 
                              $hbg=str_replace(" ","",$key->nama_poli);
                              $handle =  glob($folder."/*.*");
                              if (isset($handle[0]) && $handle !=NULL) {
                                echo '<a href="'.base_url().$handle[0].'" target="_blank">';
                                echo '<img class="d-block w-100" width="100" height="120" src="'.base_url().$handle[0].'" alt="'.$key->nama_role.'"></a>';
                              }
                            ?>
                          </td>
                          ...
                          <td class="text-center" >
                            ...
                            <a href="<?php $dxc=$this->encryption->encrypt(base64_encode($key->id)); $ff=str_replace(array('+','/','='),array('-','_','~'),$dxc); echo base_url();?>role/edit/<?php echo $ff; ?>">
                            <button type="button" class="btn btn-warning btn-sm" data-toggle="tooltip" title="Edit Role">
                            ...
                            <button type="button" class="btn btn-danger btn-sm" data-toggle="tooltip" title="Hapus Nomor" id="hapus"  data-nama_role="<?php echo $key->nama_role;?>" data-id="<?php echo $key->id;?>" >
...
$(document).on('click','#hapus',function(){
        ...
        var id=$(this).data('id');
        var no=$(this).data('nama_role');
        var text="Hapus Mobil Nomor : "+no+;
        $("#mp-tex").text(text);
        $("#id_del").val(id);
        $("#nopol").val(no);
        $("#nmfile").val(mk);
