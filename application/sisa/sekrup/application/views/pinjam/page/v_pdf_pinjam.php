...
$drv=$dataitem['drv'];
$ket=$dataitem['ket'];
$item=$dataitem['nopol']." ".$dataitem['nmerk'];
$penumpang=$dataitem['penumpang'];
...
function Header()
{
    ...
    // Move to the right
    $this->SetXY(10,7);
    $this->Cell(190,5,'e-FORMULIR PEMINJAMAN KENDARAAN KANTOR',0,2,'C');
/*    $this->SetX(40);
...
$pdf->SetX(10);
$pdf->Cell(190,6,'Dengan ini memberikan peminjaman kendaraan kantor dengan rincian sebagai berikut :',0,1,'L');
...
$pdf->Cell(60,5,$dhp->nm_bid."(".$dhp->username.")",0,1,'L');
 ...
 $nmdrvr=namaSPT($drv,date('Y'));
$pdf->SetXY(12,$pdf->GetY());
$pdf->Cell(60,5,'Penanggung Jawab/Driver',0,0,'L');
$pdf->Cell(5,5,':',0,0,'L');
$pdf->Cell(60,5,$nmdrvr,0,1,'L');

$pdf->SetXY(12,$pdf->GetY());
$pdf->Cell(60,5,'Nama Penumpang/Tim',0,0,'L');
$pdf->Cell(5,5,':',0,0,'L');
$apen=explode("-", trim($penumpang,"-"));
$nnmpen=null;
for ($dh=0; $dh < count($apen) ; $dh++) { 
  $nnmpen=$nnmpen." -".namaSPT($apen[$dh],date('Y'));
}
$pdf->MultiCell(120,5,trim($nnmpen,"/"),0,'L');
