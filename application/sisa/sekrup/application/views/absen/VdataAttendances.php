...
<body class="bg-gray-100">
    <div class="container mx-auto mt-10">
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <div class="flex justify-between px-4 py-3">
                <div class="flex items-center">
                    <label for="itemsPerPage" class="mr-2">Items:</label>
                    <select id="itemsPerPage" class="border rounded px-2 py-1"></select>
                    <div id="totalUserCount" class="mt-4 text-sm text-gray-500 ml-4">Total User: 0</div>
                </div>
            ...
            <table id="userTable" class="min-w-full bg-white border border-gray-300">
                <thead class="border">
                    <tr>
                        ...
                        <th colSpan="3" class="text-center p-4 border border-t-0">
                            Jam Kerja
                        </th>
                    ...
                    <tr>
                        <th class="text-center border border-t-0">Masuk</th>
                        <th class="text-center border border-t-0">Keluar</th>
                        <th class="text-center border border-t-0">total Jam</th>
                    </tr>
                </thead>
                <tbody id="userList"></tbody>
            </table>
    ...
    <script>
        ...
        const fetchData = async () => { 
            try {
                const response = await axios.post('<?= site_url('absen/AttendanceController/getAllData') ?>', {
                    action: 'getAllData'
                });

                // Log respons untuk memeriksa struktur datanya
                console.log(response.data);

                // Cek apakah attendance dan mahasiswa ada
                if (response.data.attendance && response.data.mahasiswa) {
                    registerData = response.data.attendance.map(attendance => {
                        const student = response.data.mahasiswa.find(m => m.nim === attendance.nim);
                        return {
                            name: attendance.name,
                            nim: attendance.nim,
                            timein: attendance.timein,
                            timeot: attendance.timeot,
                            kegiatan: attendance.kegiatan,
                            univ: student ? student.mhs_univ : 'Unknown',
                            jurusan: student ? student.mhs_jurusan : 'Unknown',
        ...
        const renderUsers = () => {
            ...
            // Memastikan registerData ada dan terisi
            if (!registerData || registerData.length === 0) {
                userList.innerHTML = "<tr><td colspan='9' class='py-2 px-4 text-left'>Tidak ada data pengguna untuk ditampilkan.</td></tr>";
                document.getElementById("totalUserCount").textContent = "Total User: 0";
                return;
            }

            // Mengambil semua pengguna dari registerData
            const start = (currentPage - 1) * itemsPerPage;
            const displayedUsers = registerData.slice(start, start + itemsPerPage);

            // Menghasilkan HTML untuk pengguna yang ditampilkan
            if (displayedUsers.length === 0) {
                userList.innerHTML = "<tr><td colspan='9' class='py-2 px-4 text-left'>Tidak ada pengguna untuk ditampilkan.</td></tr>";
            } else {
                userList.innerHTML = displayedUsers.map((user, index) => {
                    const totalHours = calculateTotalHours(user.timein, user.timeot);
                    return `
                        <tr>
                            <td class="py-2 px-4 text-left">${start + index + 1}</td>
                            <td class="py-2 px-4 text-left">${user.name}</td>
                            <td class="py-2 px-4 text-left">${user.date}</td>
                            <td class="py-2 px-4 text-center">${user.timein}</td>
                            <td class="py-2 px-4 text-center">${user.timeot}</td>
                            <td class="py-2 px-4 text-left">${totalHours} jam</td>
                            <td class="py-2 px-4 text-left">${user.kegiatan}</td>
                            <td class="py-2 px-4 text-left">${user.univ}</td>
                            <td class="py-2 px-4 text-left">${user.jurusan}</td>
            ...
            // Mengupdate total pengguna
            document.getElementById("totalUserCount").textContent = `Total User: ${registerData.length}`;
        };
        ...
        const sortByName = () => {
            sortOrderName = sortOrderName === 'asc' ? 'desc' : 'asc';
            registerData.sort((a, b) => sortOrderName === 'asc' ? a.name.localeCompare(b.name) : b.name.localeCompare(a.name));
            renderUsers();
        };

        const sortByDate = () => {
            sortOrderDate = sortOrderDate === 'asc' ? 'desc' : 'asc';
            registerData.sort((a, b) => sortOrderDate === 'asc' ? new Date(a.date) - new Date(b.date) : new Date(b.date) - new Date(a.date));
            renderUsers();
        };
                    ...
                    if (filterBy === 'name') {
                        ...
                        autoTable(doc, {
                            ...
                            body: datesInMonth.map((date, index) => {
                                ...
                                // Calculate total hours
                                const hours = attendanceOnDate ? calculateTotalHours(attendanceOnDate.timein, attendanceOnDate.timeot) : 0;
                                totalHours += hours;
                        ...
                        autoTable(doc, {
                            ...
                            body: filteredAttendanceData.map((item, index) => {
                                const hours = calculateTotalHours(item.timein, item.timeot);
                                totalHours += hours;
    ...
    const handleExport = () => {
        setShowExcelButtons(true);
        // Cek nilai bulan dan tahun sebelum melanjutkan
        const month = parseInt(selectedMonth, 10);
        const year = parseInt(selectedYear, 10);

        // Pastikan kedua nilai terisi
        if (!month || !year) {
            alert("Please select both month and year");
            return;
        }
        // Panggil fungsi ekspor dengan bulan dan tahun yang dipilih
        exportToOnlineSpreadsheet(month, year);
        setShowExcelButtons(false);
    ...
    const exportToOnlineSpreadsheet = (selectedMonth, selectedYear) => {
        ...
        const totalSheet = XLSX.utils.json_to_sheet(summaryData);
        // Otomatis lebar kolom (auto-width)
         // Definisikan lebar kolom berdasarkan panjang header
        const headers = Object.keys(summaryData[0]);
        const colWidths = headers.map(header => ({ wch: header.length + 5 })); // Menyesuaikan lebar kolom dengan header
        ...
        uniqueNames.forEach(name => {
            ...
            const monthlyAttendanceData = dateRange.map(date => {
                const formattedDate = date.toISOString().split('T')[0]; // Format date as YYYY-MM-DD
                const dailyAttendance = attendanceData.filter(att => {
                    const date = new Date(att.date).toISOString().split('T')[0]; // Mengambil hanya bagian tanggal
                    return att.name === name && date === formattedDate;
                });
            ...
            const monthlySheet = XLSX.utils.json_to_sheet(monthlyAttendanceData);
            // Set lebar kolom berdasarkan panjang header (untuk setiap sheet)
            const headersMonthly = Object.keys(monthlyAttendanceData[0]);
        ...
        document.getElementById('exportBtn').addEventListener('click', exportToOnlineSpreadsheet);
    </script>
</body>
</html>
